...
def args_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="mnist",
        choices=["mnist", "fmnist", "cifar10", "svhn", "cmapss"],
    ...
    # CMAPSS-specific settings
    parser.add_argument(
        "--cmapss_subset",
        type=str,
        default="fd001",
        choices=["fd001", "fd002", "fd003", "fd004"],
        help="CMAPSS sub-dataset to use",
    )
    parser.add_argument(
        "--window_size",
        type=int,
        default=30,
        help="sliding window size for CMAPSS sequences",
    )
    parser.add_argument(
        "--stride",
        type=int,
        default=1,
        help="sliding window stride for CMAPSS sequences",
    )
    parser.add_argument(
        "--pred_horizon",
        type=int,
        default=1,
        help="prediction horizon (RUL at window_end + pred_horizon, default 1)",
    )
    parser.add_argument(
        ...
        help="normalization method for CMAPSS sensor data",
    )
    parser.add_argument(
        "--rul_cap",
        type=int,
        default=130,
        help="RUL cap value (piecewise-linear RUL target, set to 0 to disable)",
    )
    parser.add_argument(
        "--cmapss_data_dir",
        type=str,
        default=None,
        help="directory containing CMAPSS txt files (defaults to dir_data/CMAPSS/)",
    )
    parser.add_argument(
        "--loss_type",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="fedavg",
        help="aggregation / fusion method",
    )
    parser.add_argument(
        "-dm",
        "--dir_model",
