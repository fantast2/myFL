...
def get_global_test_data_loader(
    ...
    if dataset == "cmapss":
        dl = getattr(get_client_data_loader, "_cmapss_global_test_dl", None)
        if dl is None:
            raise RuntimeError(
                "CMAPSS global test loader not available. "
                "Call get_client_data_loader with dataset='cmapss' first."
            )
        return dl
...
def get_client_data_loader(
    ...
    batch_size: int,
    cmapss_subset: str = "fd001",
    window_size: int = 30,
    stride: int = 1,
    pred_horizon: int = 1,
    normalization_method: str = "standard",
    rul_cap: int = 130,
    cmapss_data_dir: str = None,
    ...
    if dataset == "cmapss":
        ...
        _data_dir = cmapss_data_dir or os.path.join(dir_data, "CMAPSS/")
        if not os.path.isdir(_data_dir):
            raise FileNotFoundError(
                f"CMAPSS data directory not found: {_data_dir}\n"
                f"Please place train_FD001.txt, test_FD001.txt, RUL_FD001.txt, etc. in {_data_dir}"
            )

        # Inform model factory of input dimensions
        num_feat = get_cmapss_num_features(_data_dir, cmapss_subset)
        util_model.set_cmapss_dims(num_features=num_feat, window_size=window_size)

        client_data_loader, _global_test_dl = build_cmapss_client_data(
            data_dir=_data_dir,
            subset=cmapss_subset,
            num_clients=num_clients,
            partition_type=partition_type,
            partition_beta=partition_beta,
        ...
        # store global test loader for later use
        get_client_data_loader._cmapss_global_test_dl = _global_test_dl
...
# storage for the CMAPSS global test loader
get_client_data_loader._cmapss_global_test_dl = None
